...
namespace _20T1020550.BusinessLayers
{
    ...
    {
        private static IUserAccountDAL emloyeeAccountDB;

        private static IUserAccountDAL customerAccountDB;
        static UserAccountService()
        {
            string connectionString = ConfigurationManager.ConnectionStrings["DB"].ConnectionString;
            emloyeeAccountDB = new DataLayers.SQLServer.EmployeeAccountDAL(connectionString);
        ...
        public static UserAccount Authorize(AccountTypes accountType, string userName, string password)
        {
            if (accountType == AccountTypes.Employee)
                return emloyeeAccountDB.Authorize(userName, password);
            else
        ...
        {
            if (accountType == AccountTypes.Employee)
                return emloyeeAccountDB.ChangePassword(userName, oldPassword, newPassword);
            else
